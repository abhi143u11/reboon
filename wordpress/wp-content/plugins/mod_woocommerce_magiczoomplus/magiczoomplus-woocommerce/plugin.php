...
function WooCommerce_MagicZoomPlus_send_stat($action = '') {
    ...
    if('working' == 'v6.6.16' || 'working' == 'v5.2.3') {
        return;
    }
    ...
    $path = "api/stat/?action={$action}&tool_name=magiczoomplus&license=commercial&tool_version=v5.2.3&module_version=v6.6.16&platform_name=woocommerce&platform_version={$platformVersion}&url={$url}";
...
function WooCommerce_MagicZoomPlus_add_videos_product_data_fields() {
    ...
    if ( metadata_exists( 'post', $post->ID, '_provide_videolinks_field' ) ) {
        $data = unserialize(get_post_meta( $post->ID, '_provide_videolinks_field', true ));
...
function  magictoolbox_WooCommerce_MagicZoomPlus_init() {
    ...
    delete_option("WooCommerceMagicZoomPlusCoreSettings");
        add_option("WooCommerceMagicZoomPlusCoreSettings", $allParams);
    }
    ...
    if (!$coreClass->params->checkValue('page-status','No','product')) {
        add_filter( 'woocommerce_locate_template', 'WooCommerce_MagicZoomPlus_locate_template', 10, 3 );
    }
...
function WooCommerce_MagicZoomPlus_import() {
    ...
    if (array_key_exists('tool', $arr) && $tool == $arr['tool']) {
        ...
        if (array_key_exists('core', $arr)) {
            ...
            delete_option("WooCommerceMagicZoomPlusCoreSettings");
            add_option("WooCommerceMagicZoomPlusCoreSettings", $settings);
        }
...
function magictoolbox_WooCommerce_MagicZoomPlus_links( $links, $file ) {
    $fileName = 'mod_woocommerce_magiczoomplus_commercial/mod_woocommerce_magiczoomplus.php';
...
function magictoolbox_WooCommerce_MagicZoomPlus_config_page_menu() {
    if(function_exists("add_menu_page")) {
        $page = add_menu_page(__("Magic Zoom Plus for WooCommerce"), __("Magic Zoom Plus for WooCommerce"), "manage_options", "WooCommerceMagicZoomPlus-config-page", "WooCommerceMagicZoomPlus_config_page", plugin_dir_url( __FILE__ )."core/admin_graphics/icon.png");
        add_submenu_page( "WooCommerceMagicZoomPlus-config-page", 'Settings', 'Settings', 'manage_options', "WooCommerceMagicZoomPlus-config-page" );
        add_action('admin_print_scripts-' . $page, 'WooCommerce_MagicZoomPlus_add_admin_src_to_menu_page');
    }

    if(function_exists("add_submenu_page")) {

        $license_page = add_submenu_page("WooCommerceMagicZoomPlus-config-page", "License", "License", "manage_options", "WooCommerceMagicZoomPlus-license-page", "WooCommerce_MagicZoomPlus_license_page");
        add_action('admin_print_scripts-' . $license_page, 'WooCommerce_MagicZoomPlus_add_admin_src_to_license_page');
        $import_export_page = add_submenu_page("WooCommerceMagicZoomPlus-config-page", "Backup / Restore", "Backup / Restore", "manage_options", "WooCommerceMagicZoomPlus-import-export-page", "WooCommerce_MagicZoomPlus_import_export_page");
...
function  magictoolbox_WooCommerce_MagicZoomPlus_callback($matches) {
    ...
    if ($is_main) { //if it is MAIN IMAGE
        ...
        /*$img_name = str_replace(site_url('','http'),'',$thumb);
        if ($img_name == $thumb) { //maybe https
      $img_name = str_replace(site_url('','https'),'',$thumb);
        }
        if ($img_name == $thumb) {
    ...
    WooCommerce_MagicZoomPlus_get_product_variations(); //call only for onload variation check
        ...
        if (!$cat) {
            ...
            $result = $plugin->getMainTemplate(compact('img','thumb','thumb2x','id','title','description','additionalDescription','link'));
        }
        ...
        if (!$plugin->params->checkValue('create-main-image-selector','No') && !isset($GLOBALS['MAGICTOOLBOX_'.strtoupper('magiczoomplus').'_MAIN_IMAGE_SET'])) {
            ...
            if (isset($title)) { $alt = $title; } else { $alt = ''; }
            //$GLOBALS['MAGICTOOLBOX_'.strtoupper('magiczoomplus').'_MAIN_IMAGE_SELECTOR'] = $plugin->getSelectorTemplate(compact('alt','img','medium','thumb','id')); //save main image selector to globals
...
function WooCommerce_MagicZoomPlus_get_product_image($title,$size = 'thumb') {
...
function WooCommerce_MagicZoomPlus_get_post_attachments()  {
    global $wp_query;
    $plugin = $GLOBALS["magictoolbox"]["WooCommerceMagicZoomPlus"];
    $post_id = $wp_query->post->ID;
    $attachments = array();
...
function WooCommerce_MagicZoomPlus_get_product_variations ($product_id = false) {
    ...
    if ( $product->is_type('variable')) {
        ...
        if (is_array($variations) && count($variations) > 0) {
            ...
            $a = str_replace('<a','<a onclick="onMagicZoomPlusSelectorClick(this);" ',$a);
...
function WooCommerce_MagicZoomPlus_get_prepared_selectors () {
    require_once(dirname(__FILE__) . '/core/magictoolbox.imagehelper.class.php');
    ...
    $selectors = array();
    $plugin = $GLOBALS['magictoolbox']['WooCommerceMagicZoomPlus'];
    $attachments = WooCommerce_MagicZoomPlus_get_post_attachments();
    ...
    if (isset($main_selector)) {
        ...
        if (count($attachments) == 1) {
          ...
          if (basename($attachments[$att_keys[0]]->guid) == basename($test_link)) return false;
        }
    } else {
           $test_link = false;
    }
...
function WooCommerce_MagicZoomPlus_get_containers_data($thumbs = array(), $post_id = false) {
    ...
    if (metadata_exists( 'post', $post_id, '_magic360_data' ) && magictoolbox_WooCommerce_MagicZoomPlus_check_plugin_active('magic360') ){
        ...
        if(!empty($magic360_plugin) && $magic360_plugin->params->checkValue('page-status','Yes') ){
            ...
            if(!empty($magic360_image_gallery)){
                ...
                $magic360_selector = '<a data-magic-slide-id="360" style="display:inline-block;" class="m360-selector" title="360" href="#" onclick="return false;"><img src="'.WooCommerce_MagicZoomPlus_get_product_image($magic360_selector_path,'selector').'" alt="360" /></a>';
                ...
                foreach($magic360_image_gallery as $i => $image_id) {
                    ...
                    $GLOBALS['magic360images'][$i] = array(
                        'medium' => WooCommerce_Magic360_get_product_image($image_src,'thumb'),
                        'img' => WooCommerce_Magic360_get_product_image($image_src,'original'),
                    );
                }
