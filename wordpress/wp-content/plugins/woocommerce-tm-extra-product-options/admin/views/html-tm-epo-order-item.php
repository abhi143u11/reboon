...
<tr class="tm-order-line-option item <?php echo apply_filters( 'woocommerce_admin_html_order_item_class', ( ! empty( $class ) ? $class : '' ), $item, $order ); ?>" data-order_item_id="<?php echo $item_id; ?>" data-tm_item_id="<?php echo esc_attr($item_id);?>" data-tm_key_id="<?php echo esc_attr($key);?>">
    ...
    do_action( 'woocommerce_admin_order_item_values', $_product, false, 0 ); 
    ...
    <td class="item_cost" width="1%" data-sort-value="<?php echo esc_attr( $order->get_item_subtotal( $item, false, true ) ); ?>">
        <?php 
            if(empty($item_meta['tm_has_dpd']) && empty($item_meta['_tm_has_dpd'])){
                if ($epo['quantity'] <=0){
                    echo '<div class="view">'.wc_price( 0 ).'</div>';
                    if ($epo_edit_cost)
                    echo '<div class="edit" style="display: none;"><input type="number" name="tm_epo['.$item_id.']['.$key.'][price]" placeholder="0" value="0" data-qty="0" size="4" class="price" /></div>';
                }else{
                    echo '<div class="view">'.
                    ($epo_is_fee
                        ?wc_price(  (float) $epo['price'] )
                        :wc_price( tc_get_price_excluding_tax($_product,array('qty'=>1,'price'=>$epo['price'])) / $epo['quantity'] )
                        )
                    
                    .'</div>';  
                    if ($epo_edit_cost)
                    echo '<div class="edit" style="display: none;"><input type="number" name="tm_epo['.$item_id.']['.$key.'][price]" placeholder="0" value="'.( tc_get_price_excluding_tax($_product,array('qty'=>1,'price'=>$epo['price']))/$epo['quantity'] ).'" data-qty="'.( tc_get_price_excluding_tax($_product,array('qty'=>1,'price'=>$epo['price']))/$epo['quantity']  ).'" size="4" class="price" /></div>';
                }
            }else{
    ...
    <td class="quantity" width="1%">
        ...
        <div class="edit" style="display: none;">
            ...
            <input type="number" step="1" min="0" autocomplete="off" name="tm_epo[<?php echo absint( $item_id ); ?>][<?php echo $key; ?>][quantity]" placeholder="0" value="<?php echo $epo['quantity']; ?>" data-qty="<?php echo $epo['quantity']; ?>" size="4" class="quantity" />
        ...
        <div class="view">
            <?php 
                if(empty($item_meta['tm_has_dpd']) && empty($item_meta['_tm_has_dpd'])){
                    ...
                    ($epo_is_fee
                        ?wc_price(  (float) $epo['price'] )
                        :wc_price(  (float) tc_get_price_excluding_tax($_product,array('qty'=>1,'price'=>$epo['price'])) * (float) $item_meta['_qty'][0] )
                        )
    ...
    <?php 
        if ( ! empty( $tax_data ) ) {
            ...
            if ( $epo_is_fee ){
                ...
                $epo_line_taxes = WC_Tax::calc_tax( (float) $epo['price'] * (float) $item_meta['_qty'][0], $tax_rates, get_option( 'woocommerce_prices_include_tax' )==='yes' );    
            }
                    ...
                    <div class="view">
                        <?php 
                            if (isset($epo_line_taxes[$tax_item_id])){
                                ...
                                echo wc_price( wc_round_tax_total( $tax_price ), array( 'currency' => (is_callable( array($order, 'get_currency'))?$order->get_currency():$order->get_order_currency()) ) );
                            }else{
                                echo '&ndash;';
                            }
                        ?>
