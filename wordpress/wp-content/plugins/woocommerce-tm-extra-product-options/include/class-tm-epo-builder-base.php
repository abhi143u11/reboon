			...
			if ( is_array( $options ) ) {
				...
				if ( taxonomy_exists( sanitize_title( $name ) ) ) {
					...
					foreach ( $options as $option ) {
						$optiont = TM_EPO_HELPER()->html_entity_decode($option);
	...
	private function builder_sub_options( $options=array(), $name="multiple_selectbox_options", $counter=NULL , $default_value=NULL) {
		...
		$out  = "<div class='tm-row nopadding multiple_options tc-clearfix'>"
			. "<div class='tm-cell col-auto tm_cell_move'>".
				TM_EPO_HTML()->tm_make_button( array(
					"text"=>"<i class='tcfa tcfa-sort'></i>",
					"tags"=>array( "href"=>"#move", "class"=>"button button-secondary button-small tm-hidden-inline" )
					), 0 ).
				TM_EPO_HTML()->tm_make_button( array(
					"text"=>"<i class='tcfa tcfa-angle-up'></i>",
					"tags"=>array( "href"=>"#move", "class"=>"button button-secondary button-small tm-hidden-inline" )
		...
		foreach ( $options["title"] as $ar=>$el ) {
			...
			$out .= "<div class='tm-cell col-2 tm_cell_images'>".TM_EPO_HTML()->tm_make_field( $o["image"], 0 ).TM_EPO_HTML()->tm_make_field( $o["imagec"], 0 ).TM_EPO_HTML()->tm_make_field( $o["imagep"], 0 ).TM_EPO_HTML()->tm_make_field( $o["imagel"], 0 )."</div>";
