                ...
                if(isset($epo['mode']) && $epo['mode']=='local'){
                    if(isset($epo['is_taxonomy'])){
                        if($epo['is_taxonomy']=="1"){
                            ...
                            $attributes = maybe_unserialize( tc_get_post_meta( floatval(TM_EPO_WPML()->get_original_id( $cart['product_id'] )), '_product_attributes', true ) );
                            $wpml_attributes = maybe_unserialize( tc_get_post_meta( floatval(TM_EPO_WPML()->get_current_id( $cart['product_id'] )), '_product_attributes', true ) );
