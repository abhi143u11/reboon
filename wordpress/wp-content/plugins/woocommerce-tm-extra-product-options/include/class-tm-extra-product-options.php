...
final class TM_Extra_Product_Options {
	...
	public function add_plugin_actions(){
		...
		add_filter( 'woocommerce_display_item_meta', array( $this,'tm_woocommerce_display_item_meta'), 10, 3 );
	...
	public function tm_get_price_html($price='',$product=false){
		...
		if (!$product->tc_min_max){
			$minmax = $this->add_product_tc_prices($product);
		}
		$type = $product->get_type();
		$min = floatval( apply_filters( 'wc_epo_options_min_price', $product->tc_min_price, $product, $price ) );
		$max = floatval( apply_filters( 'wc_epo_options_max_price', $product->tc_max_price, $product, $price ) );
		...
		if ($type == 'variable' || $type == 'variable-subscription'){
			...
			$price     = $min_price !== $max_price 
				? !$use_from 
					...
					: (function_exists('wc_get_price_html_from_text')?wc_get_price_html_from_text():$product->get_price_html_from_text()) . wc_price( $min_price )
				: wc_price( $min_price );
			...
			if ( $product->is_on_sale() ) {
				...
				$regular_price     = $min_regular_price !== $max_regular_price 
					? !$use_from
						...
						: (function_exists('wc_get_price_html_from_text')?wc_get_price_html_from_text():$product->get_price_html_from_text()) . wc_price( $min_regular_price )
			...
			if ( $this->tc_get_price($product) > 0 ) {
				...
				if ( $product->is_on_sale() && $this->tc_get_regular_price($product) ) {
					if ($use_from && ($max>0 || $max>$min)){
						$price .= (function_exists('wc_get_price_html_from_text')?wc_get_price_html_from_text():$product->get_price_html_from_text()).wc_price( $display_price );
					}else{
						$price .= (function_exists('wc_format_price_range')?wc_format_price_range($display_regular_price, $display_price):$product->get_price_html_from_to( $display_regular_price, $display_price ));
					}
				...
				if ( $product->is_on_sale() && $this->tc_get_regular_price($product) ) {
					if ($use_from && ($max>0 || $max>$min)){
						$price .= (function_exists('wc_get_price_html_from_text')?wc_get_price_html_from_text():$product->get_price_html_from_text()).wc_price( ($min>0)?$min:0 );
					}else{
					
						$price .= (function_exists('wc_format_price_range')?wc_format_price_range($display_regular_price, __( 'Free!', 'woocommerce' )):$product->get_price_html_from_to( $display_regular_price, __( 'Free!', 'woocommerce' ) ));
	...
	public function tm_post_class($classes=""){
		...
		if ( $post_id && ('product' == get_post_type( $post_id ) || $this->wc_vars['is_product']) ) {
	...
	public function tm_woocommerce_before_add_to_cart_button(){
		...
		if ($product && is_object($product) && property_exists($product, "id") ){
			echo '<input type="hidden" class="tc-add-to-cart" name="tcaddtocart" value="'.esc_attr( tc_get_id($product) ).'" />';
		}	
	...
	/** Adds options to the array of items/products of an order **/
	public function tm_woocommerce_display_item_meta($html='', $item=array(), $args=array()){
		...
		foreach ( $item_meta as $meta ) {
			$has_epo = is_object($meta) && $meta->key=='_tmcartepo_data';
			if ($has_epo){
				$epos = maybe_unserialize($meta->value);
				$current_meta=$meta;
				break;
			}
		}
		...
		if ($order){
			$mt_prefix = is_callable( array($order, 'get_currency'))?$order->get_currency():$order->get_order_currency();
		
		}

		$current_product_id = tc_get_id($item->get_product());
						...
						if ( !empty( $check['ext'] ) ) {
                			...
                			if ( in_array( $check['ext'], $image_exts ) ){
                				$display_value ='<span class="cpf-img-on-cart"><img alt="" class="attachment-shop_thumbnail wp-post-image epo-option-image epo-upload-image" src="'.
									apply_filters("tm_image_url",$original_value).'" /><span class="cpf-data-on-cart">'. $epo['value'] .'</span></span>';
								$epo['value']=$display_value;
                			}
        				}
					}
		...
		foreach ($_items_to_add as $uniquid => $element) {
			foreach ($element as $key => $value) {
				...
				$value = $args['autop'] ? wp_kses_post( wpautop( make_clickable( $value ) ) ) : wp_kses_post( make_clickable( $value ) );
				$strings[] = '<strong class="wc-item-meta-label">' . wp_kses_post( $key ) . ':</strong> ' . $value;
			}
		}
		if ( $strings ) {
			if (substr($html, 0, strlen($args['before'])) == $args['before']) {
			    $html = substr($html, strlen($args['before']));
			}
			if (substr($html,-strlen($args['after']))===$args['after']){
				$html = substr($html, 0, strlen($html)-strlen($args['after']));
			}
			$old_html=explode( $args['separator'], $html );
			$strings = array_merge($old_html, $strings);
				...
				foreach ($_items_to_add as $uniquid => $element) {
					foreach ($element as $key => $value) {
						...
						if (is_array($items[$item_id])){
							...
							//echo'<pre>';var_dump($items[$item_id]);echo'</pre>';
						}
	...
	public function tm_epo_option_name( $label="", $args=null, $counter=null, $value=null, $vlabel=null ) {
		...
		if ($this->tm_epo_show_price_inside_option=='yes' && $value!==null && $vlabel!==null && empty($args['rules_type'][$value])){
	...
	public function add_to_cart_text( $text="" ) {
		...
		if ( $this->tm_epo_enable_in_shop=="no" 
			...
			$cpf=$this->get_product_tm_epos(tc_get_id($product));
	...
	public function add_cart_item( $cart_item=array() ) {
		...
		$cart_item['tm_epo_product_original_price']=$cart_item['data']->get_price();
		...
		if ( ! empty( $cart_item['tmcartepo'] ) ) {
			...
			if (is_array($cart_item['tmcartepo'] )){
				foreach ( $cart_item['tmcartepo'] as $tmcp ) {//var_dump('tmcp='. $tmcp['price'] );
					if(isset($tmcp['subscription_fees'])){
						continue;
					}
					$_price_type=$this->get_element_price_type($tmcp);
					/*echo'<pre>';    
					var_dump( 'to_currency '. $to_currency  );
					var_dump( 'check1 '. isset($tmcp['price_per_currency'])  );
					var_dump( 'check2 '. isset($tmcp['price_per_currency'][$to_currency])  );
					var_dump( 'check3 '. ($tmcp['price_per_currency'][$to_currency]!='')  );
					var_dump( 'tmcp[price_per_currency][$to_currency] '. $tmcp['price_per_currency'][$to_currency]  );
					echo'</pre>';	*/
					...
					if(isset($tmcp['price_per_currency']) && isset($tmcp['price_per_currency'][$to_currency]) && $tmcp['price_per_currency'][$to_currency]!=''){
						...
						//echo'<pre>';var_dump( ' tmcp price BEFORE 1= '. $tmcp['price']);echo'</pre>';						
						...
						$tmcp_prices += apply_filters( 'woocommerce_tm_epo_price_add_on_cart', 
							$tmcp['price'], 
							 $_price_type);
						//echo'<pre>';var_dump( ' tmcp price BEFORE 2= '. $tmcp['price']);echo'</pre>';
						if ($_price_type=="percentcurrenttotal"){
			...
			if (!empty($cart_item['tmpost_data'])){
				...
				//unset($cart_item['tmcartepo']);
				//unset($cart_item['tmsubscriptionfee']);
				//unset($cart_item['tmcartfee']);
				//unset($cart_item['tmcartepo_bto']);
				//unset($cart_item['tmpost_data']);
				...
				$post_data['cpf_product_price'] = $cart_item['data']->get_price();
			...
			if (is_array($cart_item['tmcartepo'] )){
				...
				foreach ( $cart_item['tmcartepo'] as $tmcp ) {
					...
					$_price_type=$this->get_element_price_type($tmcp);
					if(isset($tmcp['price_per_currency']) && isset($tmcp['price_per_currency'][$to_currency]) && $tmcp['price_per_currency'][$to_currency]!=''){
						$tmcp_pricesf=apply_filters( 'woocommerce_tm_epo_price_per_currency_diff', (float)wc_format_decimal($tmcp['price_per_currency'][$to_currency],false,true), $to_currency );
						//$tmcp_pricesf = $tmcp['price'];
						//echo'<pre>';var_dump( ' tmcp price AFTER 1= '. $tmcp_pricesf);echo'</pre>';						
						/*if ($_price_type=="percentcurrenttotal"){
							$tmcp_variable_prices2 += $tmcp['price'];
						}elseif ($_price_type=="percent"){
							$tmcp_variable_prices2 += $tmcp['price'];
						...
						//$tmcp['price']=(float)wc_format_decimal($tmcp['price'],false,true);
						$tmcp_pricesf = apply_filters( 'woocommerce_tm_epo_price_add_on_cart', 
							$tmcp['price'], 
							 $_price_type);
						//echo'<pre>';var_dump( ' tmcp price AFTER 2= '. $tmcp_pricesf);echo'</pre>';
						/*if ($_price_type=="percentcurrenttotal"){
							$tmcp_variable_prices += $tmcp['price'];
						}elseif ($_price_type=="percent"){
							$tmcp_variable_prices2 += $tmcp['price'];
...
//echo'<pre>';var_dump( ' tc_get_woocommerce_currency= '. tc_get_woocommerce_currency());echo'</pre>';			
...
//echo'<pre>';var_dump( ' tmcp_static_prices= '. $tmcp_static_prices);echo'</pre>';
//echo'<pre>';var_dump( ' tmcp_variable_prices= '. $tmcp_variable_prices);echo'</pre>';
//echo'<pre>';var_dump( ' tmcp_variable_prices2= '. $tmcp_variable_prices2);echo'</pre>';


			$cart_item['tm_epo_options_prices']=$tmcp_prices;

//echo'<pre>';var_dump( ' tmcp_prices = '. $tmcp_prices );echo'</pre>';


//		echo'<pre>';var_dump( ' price= '. $cart_item['data']->price);echo'</pre>';
//		echo'<pre>';var_dump( ' get_price= '. $cart_item['data']->get_price());echo'</pre>';

			$cart_item['tm_epo_product_original_price']=$cart_item['data']->get_price();
//apply_filters('tm_epo_price_per_currency_diff',(wc_format_decimal($cart_item['tm_epo_product_original_price']) + wc_format_decimal($tmcp_prices)) )

			$price1 = wc_format_decimal (apply_filters('wc_epo_product_price_correction', $tmcp_prices));
			$price2 = wc_format_decimal (apply_filters('wc_epo_product_price_correction', wc_format_decimal($cart_item['tm_epo_product_original_price']) + wc_format_decimal($tmcp_prices)));

//echo'<pre>';var_dump( ' price2= '. $price2);echo'</pre>';
			...
			if ( apply_filters( 'wc_epo_adjust_price', true, $cart_item ) ){
				if (!empty($cart_item['epo_price_override']) && $tmcp_prices>0){
					...
					//$cart_item['data']->price = $price1;
				}else{//var_dump(wc_format_decimal($cart_item['data']->price)  );
					// setting price twice for compatibility
					$cart_item['data']->set_price( $price2 );
					//var_dump(wc_format_decimal($cart_item['data']->price)  );
					//$cart_item['data']->price =  $price2;
//var_dump(wc_format_decimal($cart_item['data']->price)  );
	...
	public function get_price_for_cart($price=0,$cart_item=array(),$symbol=false,$currencies=NULL,$quantity_divide=0,$quantity=0,$price_type=""){
		...
		}//echo'<pre>';var_dump('expression = '.$price);echo'</pre>';
		//$price = apply_filters( 'wc_epo_get_current_currency_price', apply_filters( 'woocommerce_tm_epo_price_on_cart',$price),$price_type,false,$currencies );
		...
		if ( $taxable ) {
			...
			if ( $tax_display_cart == 'excl' ) {
				...
				if (floatval($price)!=0){
					$price = tc_get_price_including_tax($product,array( 'qty'=>$quantity, 'price'=>$price ));
				}
			}
		...
		if ($symbol===false){
			if ($this->tm_epo_global_price_sign=='' && $this->tm_epo_cart_field_display!="advanced"){
				$symbol="<span class='tc-plus-sign'>+</span>";
			}
			if (floatval($price)<0){
				$symbol="<span class='tc-minus-sign'>-</span>";
			}
		}
			...
			switch ($_price_type) {
				case 'percent':
					if ($cpf_product_price!==false){
						...
						if ($currency){
							$cpf_product_price=apply_filters( 'wc_epo_get_currency_price',$cpf_product_price,$currency,$_price_type,false,$current_currency,$price_per_currencies,$_price_type);
						}else{
						...
						//echo'<pre>=';var_dump($_price);echo'</pre>';echo'<pre>-';var_dump($cpf_product_price);echo'</pre>';
					}
					break;
				case 'percentcurrenttotal':
					...
					if ($_price!='' && isset($post_data[$attribute.'_hidden'])){
						...
						//echo'<pre>-';var_dump($_price);var_dump($currency);var_dump($current_currency);echo'</pre>';
//echo'<pre>-';var_dump( apply_filters( 'wc_epo_get_currency_price', floatval($post_data['tm_epo_options_static_prices']) ) );echo'</pre>';
						...
						if ($currency){
							$_price=apply_filters( 'wc_epo_get_currency_price',$_price,$currency,$_price_type,false,$current_currency,$price_per_currencies,$_price_type);
						}else{
							//$_price=apply_filters( 'wc_epo_remove_current_currency_price',$_price,"percentcurrenttotal");
						}

						
						//echo'<pre>=';var_dump($_price);echo'</pre>';
	...
	public function tm_epo_fields($product_id=0, $form_prefix="", $is_from_shortcode=false, $dummy_prefix=false) {
		...
		if ( is_array( $local_price_array ) && sizeof( $local_price_array ) > 0 ) {
			...
			$attributes = maybe_unserialize( tc_get_post_meta( floatval(TM_EPO_WPML()->get_original_id( $post_id ) ), '_product_attributes', true ) );
			$wpml_attributes = maybe_unserialize( tc_get_post_meta( $post_id, '_product_attributes', true ) );
	...
	public function custom_frontend_scripts() {
		...
		$formatted_price = str_replace('0', '{{{ data.price }}}', $zero_price);
		$suffix='';
		if ($product){
	...
	private function print_price_fields( $product_id=0, $form_prefix="", $is_from_shortcode=false) {
		...
		if ($product->get_type()=="composite" && is_callable( array($product, 'get_base_price'))){
										...
										if (is_array($meta)){
											if (isset($meta[0]) && is_array($meta[0])){
												foreach ($meta as $k => $value) {
													...
													$cart_item_meta['tmdata']['tmcartfee'][] = $element;
										...
										if (is_array($meta)){
											if (isset($meta[0]) && is_array($meta[0])){
												foreach ($meta as $k => $value) {
													...
													$cart_item_meta['tmdata']['tmcartepo'][] = $element;
	...
	public function add_cart_item_data_helper( $cart_item_meta, $product_id, $post_data = null ) {
		...
		$terms 			= get_the_terms( $product_id, 'product_type' );
		$product_type 	= ! empty( $terms ) && isset( current( $terms )->name ) ? sanitize_title( current( $terms )->name ) : 'simple';
		if ( ($product_type == 'bto' || $product_type == 'composite') && isset( $post_data[ 'add-product-to-cart' ] ) && is_array( $post_data[ 'add-product-to-cart' ] ) ) {
			$copy=array();
			foreach ( $post_data[ 'add-product-to-cart' ] as $bundled_item_id => $bundled_product_id ) {
...
//echo'<pre>';var_dump($cpf_product_price);echo'</pre>';
//return $cart_item_meta;
...
//echo'<pre>';var_dump( 'START');echo'</pre>';						
					foreach ($price_per_currencies as $currency => $price_rule) {
						...
						$currency_price=TM_EPO()->calculate_price( $post_data, 
							$copy_element, 
							$cart_item_meta['tmdata']['tmcartepo_data'][$key]['key'], 
...
//echo'<pre>';var_dump( ' currency= '. $currency.' = '.$currency_price);echo'</pre>';						
						$price_per_currency[$currency] = $currency_price;
					}

					$_price=TM_EPO()->calculate_price( $post_data, 
