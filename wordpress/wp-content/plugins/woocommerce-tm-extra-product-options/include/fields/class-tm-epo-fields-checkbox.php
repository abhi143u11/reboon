...
class TM_EPO_FIELDS_checkbox extends TM_EPO_FIELDS {
	...
	public function display_field( $element=array(), $args=array() ) {
		...
		elseif ( ( (TM_EPO()->is_quick_view() || empty($this->post_data)) && empty(TM_EPO()->cart_edit_key) )|| TM_EPO()->tm_epo_global_reset_options_after_add=="yes" ){
			$selected_value=-1;
		}
	...
	public function validate() {
		...
		foreach ( $this->tmcp_attributes as $k=>$attribute ) {
			...
			if (isset($this->epo_post_fields[$attribute]) && 
				isset($this->epo_post_fields[$attribute.'_quantity']) && 
			...
			}elseif (isset($this->epo_post_fields[$attribute]) && 
				isset($this->epo_post_fields[$attribute.'_quantity']) && 
				(intval($this->epo_post_fields[$attribute.'_quantity'])==0)){
				$passed = false;
				$message[] = sprintf( __( 'The quantity for "%s" must be greater than 0 to be added to the cart.', 'woocommerce-tm-extra-product-options' ),  $this->element['options'][$this->epo_post_fields[$attribute]], $min_quantity );
			}
