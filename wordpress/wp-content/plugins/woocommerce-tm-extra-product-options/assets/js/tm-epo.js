(function($) {
                ...
                if (get_element && get_element.length>0){
                    get_element.each(function(i,element){
                        var $element=$(element),
                            ...
                            $this_epo_container=$('.tc-extra-product-options'+$form.data('product_id_selector')+$form.data('epo_id_selector'));
    ...
    // Taxes setup
    function get_price_including_tax(price, _cart, element) {
    ...
    function get_price_excluding_tax(price, _cart, element) {
        if (isNaN(parseFloat(price))){
            price=0;
        }
        price=price*10000;
        if (_cart){
            ...
            if ( taxable && prices_include_tax=="1" ) {;
                price = parseFloat(price)-parseFloat(base_taxes_of_one*price);//parseFloat(price)/(1+(tax_rate/100));
            }
    ...
    function tm_init_epo(main_product,is_quickview,product_id,epo_id){
            ...
            if (tm_epo_js.tm_epo_global_enable_validation=="yes"){
                ...
                var validator = form.tc_validate({
                        ...
                        invalidHandler : function(event, validator) {
                            setTimeout(function() {
                                main_product.find(add_to_cart_button_selector).first().removeClass('loading').removeAttr('disabled').removeClass('fpd-disabled');
                        ...
                        submitHandler : function(form) {
                            main_product.find(add_to_cart_button_selector).first().attr('disabled','disabled');
        ...
        function apply_submit_events(e){
            ...
            if(!form_is_submit){
                setTimeout(function() {
                    main_product.find(add_to_cart_button_selector).first().removeAttr('disabled');
                }, 100 );
            }
        ...
        function goto_error_item(item){
            ...
            if(el){
                ...
                if (elsection.is('.section_popup')){
                    ...
                    //setTimeout(function() {
                        var pos=el.closest('.owl-item').index();//console.log("test"+pos);
        ...
        function tm_calculate_product_price(totals){
            ...
            if (!rules || !$cart){
                ...
                var qty_element = totals.data('qty_element'),
                    qty = parseFloat(qty_element.val()),
                    current_variation=totals.data('variation_id_selector').val();
        ...
        function tm_epo_init($cart_container,$composite_cart) {
            ...
            $totals_holder.data('variation_id_selector',current_cart.find(variation_id_selector));
            $totals_holder.data('qty_element',current_cart.find('input.qty').last());
            ...
            .on('change.cpf',  function(pass) {
                ...
                if (!field.is('.tm-epo-variation-element')){
                    ...
                    if (field.is('.use_images:checkbox, .use_images:radio') && field.attr('data-imagec')){
                        var is_replace=field.closest(".tmcp-field-wrap").find(".radio_image,.checkbox_image").first();
            ...
            }).data('tm-prev-value', $cart_container.find('input.qty').last().val());
            ...
            .on('tm-epo-update', function(pass) {
                ...
                if (show_total && qty > 0) {
                    ...
                    if (formatted_final_total && product_total_price){
                        ...
                        if (tm_epo_final_total_box=='disable_change' || tm_epo_js.tm_epo_change_variation_price=='yes'){    
                            ...
                            //if (tm_epo_final_total_box=='hide'){
            ...
            main_product.find('.tm-variation-ul-color .tmhexcolorimage-li-nowh,.tm-variation-ul-image .tmhexcolorimage-li-nowh,ul.use_images_containter .tmhexcolorimage-li-nowh').each(function(i,el){
        ...
        function tm_product_image(){
            ...
            if (tm_epo_js.tm_epo_global_product_image_selector!==''){
                var img = $(tm_epo_js.tm_epo_global_product_image_selector);
            }else{
                var img=product_element.find("a.woocommerce-main-image img, img.woocommerce-main-image,a img").not('.thumbnails img,.product_list_widget img').first();
            }
            ...
            var is_elevateZoom=img.data('elevateZoom') || false,is_elevateZoom_obj=product_element.find( 'div.product-images .woocommerce-main-image' );
            ...
            if ($(img).length>0){
                ...
                .on('tm_change_product_image',function(evt,e){
                    ...
                    var preloader=$("<div class='blockUI blockOverlay tm-preloader-img'></div>");
                    ...
                    preload_img.onload = function() {
                        ...
                        if (is_elevateZoom){                            
                            is_elevateZoom_obj.each(function() {
                                var elevateZoom = $(this).data('elevateZoom');
                                if (typeof elevateZoom != 'undefined') {
                                    elevateZoom.swaptheimage(preload_img.src,preload_img.src);
                                }
                            });
                        }
                    ...
                    if(can_show_image){
                        ...
                        /*if (image_info['imagep']){
                            clone_image.attr('title',image_info['imagep']['image_title']);
                            clone_image.attr('alt',image_info['imagep']['image_alt']);
                            if(image_info['imagep']['image_srcset'])clone_image.attr('srcset',image_info['imagep']['image_srcset']);
                            if(image_info['imagep']['image_sizes'])clone_image.attr('sizes',image_info['imagep']['image_sizes']);
                            a.attr('href',image_info['imagep']['image_link']);
                            a.attr('title',image_info['imagep']['image_caption']);
                            img.attr('title',image_info['imagep']['image_title']);
                            img.attr('alt',image_info['imagep']['image_alt']);
                        }*/
                ...
                .on('tm_restore_product_image',function(evt,e){
                    ...
                    if (is_elevateZoom){                            
                        is_elevateZoom_obj.each(function() {
                            ...
                            if (typeof elevateZoom != 'undefined') {
                                if (found) {
                                    elevateZoom.swaptheimage(current_image_replacement.attr('src'),current_image_replacement.attr('src'));
                                }else{
                                    elevateZoom.swaptheimage(img_src_original,img_src_original);
                                }                                    
                            }
                        });
                    }
        ...
        function tm_theme_specific_actions(){
            ...
            .on('click.cpf', '.quantity:not(.buttons_added) .minus, .quantity:not(.buttons_added) .plus',  function() {
                ...
                $(this).closest('.quantity').find('input.qty').trigger('change.cpf');
        ...
        function add_late_fields_prices(product_price,bid,_cart){
            ...
            $.each(late_fields_prices,function(i,field){
                ...
                if (bundleid==bid){
                    ...
                    var formatted_price = tm_set_price(price,_cart,true,false,setter),
                    ...
                    if (setter.data('pricew')!==undefined){
                        var formatted_price = tm_set_price(setter.data('pricew'),_cart,true,false,setter),
                            original_formatted_price="";
                    ...
                    "on_true" : function(){
                        ...
                        if (upload_fields){
                            var temp_floatbox = $("body").tm_floatbox({
                                ...
                                "data": $.fn.tm_template(template_engine.tc_upload_messages,{'files':upload_fields,'title':tm_epo_js.i18n_uploading_files,'message':tm_epo_js.i18n_uploading_message}),
                            });
    ...
    _document.ready(function() {
        ...
        $.ajaxPrefilter(function( options, originalOptions, jqXHR ) {
            ...
            if(originalOptions.data){
                ...
                if ( "quantity" in _data && (_data["product_id"] || _data["add-to-cart"] || _urldata["product_id"] || _urldata["add-to-cart"]) ){

                    var _pid = _data["product_id"] || _data["add-to-cart"] || _urldata["product_id"] || _urldata["add-to-cart"];
                    ...
                    if(epos.length==1){
                        ...
                        options.data = $.param( 
                            $.extend(
                                typeof originalOptions.data=="string" ?originalOptions.data.tmparseParams():originalOptions.data, 
                                epos.tm_aserializeObject(),
                                obj
                            ), false );
                    }
            ...
            for (var key in qv_container) {
                if (qv_container.hasOwnProperty(key)) {
                    if (qv_container[key].length){
                        ...
                        if(key=='jckqv_quick_view' || key=="yith_quick_view_plugin"){
