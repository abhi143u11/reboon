...
(function($) {
    ...
    String.prototype.tmparseParams = function(decode){
        return this.split("&").map(function(n){if(decode===true){n=decodeURIComponent(n);}return n = n.split("="),this[n[0]] = n[1],this}.bind({}))[0];
    };
...
(function($) {
	 ...
	 */
	$.fn.tc_set_attr = function( attr, value ) {
		if ( undefined === this.attr( 'data-tco_' + attr ) ) {
			this.attr( 'data-tco_' + attr, ( ! this.attr( attr ) ) ? '' : this.attr( attr ) );
		}
	 ...
	 */
	$.fn.tc_reset_attr = function( attr ) {
		if ( undefined !== this.attr( 'data-tco_' + attr ) ) {
			this.attr( attr, this.attr( 'data-tco_' + attr ) );
		}
	};
	...
	$.fn.tc_image_update = function( element, image ) {
		...
		var $form             	= this,
			...
			$product_img 		= $product_element.find("a img:eq(0)").not('.thumbnails img'),
		...
		if (element && image_info && $image.length>0){
			...
			$image.attr('title',image_info['imagep']['image_title']);
			$image.attr('alt',image_info['imagep']['image_alt']);
			if(image_info['imagep']['image_srcset']){
				$image.attr('srcset',image_info['imagep']['image_srcset']);
			}
			if(image_info['imagep']['image_sizes']){
				$image.attr('sizes',image_info['imagep']['image_sizes']);
			}

			//$product_img.tc_set_attr( 'src', variation.image_src );
			$product_img.tc_set_attr( 'title', image_info['imagep']['image_title'] );
			$product_img.tc_set_attr( 'alt', image_info['imagep']['image_alt'] );
			//$product_img.tc_set_attr( 'srcset', variation.image_srcset );
			//$product_img.tc_set_attr( 'sizes', variation.image_sizes );
			
			$product_link.tc_set_attr( 'href', image_info['imagep']['image_link'] );
			$product_link.tc_set_attr( 'title', image_info['imagep']['image_caption'] );
		}else{
