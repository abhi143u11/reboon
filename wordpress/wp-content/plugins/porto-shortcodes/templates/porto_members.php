<?php
$output = $title = $columns = $view = $overview = $socials = $cat = $cats = $post_in = $number = $view_more = $view_more_class = $filter = $pagination = $ajax_load = $animation_type = $animation_duration = $animation_delay = $el_class = '';
...
if ($posts->have_posts()) {
    ...
    <?php if ($view_more) : ?>
        <div class="push-top m-b-xxl text-center">
            <a class="button<?php echo $view_more_class ? ' ' . str_replace('.', '', $view_more_class) : '' ?>" href="<?php echo get_post_type_archive_link( 'member' ) ?>"><?php _e("View More", 'porto-shortcodes') ?></a>
        </div>
    <?php endif; ?>
