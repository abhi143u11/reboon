...
/*---------------TRD---------------------------*/
// если поиск выдал только 1 чехол для товара, то переход сразу на него
global $wp_query;
$posts = $wp_query->posts;
if(count($posts) == 1){	
	$titlePos = (strpos($posts[0]->guid,"shop/") !== false) ? strpos($posts[0]->guid,"shop/") + 4 : strpos($posts[0]->guid,"uncategorized/") + 13;	
	$request = get_site_url().'/shop'.substr($posts[0]->guid,$titlePos);		
...
<?php 
    ...
    $psa_desc = category_description($category_id);
    if (!empty($psa_desc)) {
        echo '<div class="row"><div class="col-md-12 psa-desc">'. $psa_desc.'</div></div>';
    }
